...
@click.option(
    ...
    "--base",
    default="",
    help="base path where the server will find custom error files."
)
...
def serve(root: str="", base: str = "", port: int=3031, watch: list[str]=[], version: bool = False):
    ...
    liveserver = LiveServer(*watch, root=root, base=base, port=port)
    try:
